...
unsigned char* read_cbor(const unsigned char* payload, int i_len) {
  ...
  nonce[14] = "\0";
  printf("Nonce is %s\n", nonce);
  dtls_decrypt(payload, 113, buffer, nonce, key, 16, A_DATA, A_DATA_LEN);
  printf("COSE decrypted\n");
  cn_cbor *cb2 = cn_cbor_decode(buffer, i_len CBOR_CONTEXT_PARAM, 0);
  if (cb2) {
